...
import {rfqItemReducer} from './reducer';
...
const Store = ({children}) => {
  const [state, dispatch] = useReducer(rfqItemReducer,initialState)
  const [rfq, setRfq] = useState(state.rfqs);
  const [rfqItem, setRfqItem] =  useState(state.rfqItems);
  const [quotes, setQuotes] = useState(state.quotes);
  const [purchaseOrders, setPurchaseOrders] = useState(state.purchaseOrders);
  const [invoices, setInvoices] = useState(state.invoices);
  const [contacts, setContacts] = useState(state.contacts);
  const [deliveryNotes, setDeliveryNotes] = React.useState(state.deliveryNotes);
    return (
